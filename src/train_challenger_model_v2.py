...
def train_svc_challenger():
    ...
    # Scale numeric features (Critical for SVM performance!)
    scaler = StandardScaler()
    X_train_scaled = scaler.fit_transform(X_train)
    X_test_scaled = scaler.transform(X_test)

    # SVC Parameters with probability=True for ROC-AUC scoring
    params = {"kernel": "rbf", "C": 1.0, "probability": True, "random_state": 42}

    print("2. Training CHALLENGER Model (SVM Classifier)...")
    with mlflow.start_run(run_name="Challenger_SVM_Classifier") as run:
        model = SVC(**params)
        ...
        # Log parameters & classification metrics to MLflow
        ...
        joblib.dump(scaler, "models/scaler_svc.pkl")
        mlflow.log_artifact("models/scaler_svc.pkl")

        joblib.dump(model, "models/model_svc.pkl")
        
        # Log and register the SVC model in MLflow
